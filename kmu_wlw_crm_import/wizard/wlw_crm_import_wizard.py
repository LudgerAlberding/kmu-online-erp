...
class WlwCrmImportWizard(models.TransientModel):
    ...
    def _create_vals(self, v, source, medium, campaign, tag, signature):
        vals = {
            ...
            "description": self._lead_description(v),
    ...
    def _update_vals(self, lead, v, signature):
        ...
        new_name = self._lead_name(v)
        if new_name and lead.name != new_name:
            vals["name"] = new_name
        ...
        products = self._append_unique(lead.x_wlw_products, v["products"])
        if products != (lead.x_wlw_products or ""):
            vals["x_wlw_products"] = products

        keywords = self._append_unique(lead.x_wlw_keywords, v["keywords"])
        if keywords != (lead.x_wlw_keywords or ""):
            vals["x_wlw_keywords"] = keywords
        ...
        if new_visit:
            ...
            visit_text = self._history(v, signature)
            if visit_text:
                if history:
                    vals["x_wlw_visit_history"] = (
                        history.strip()
                        + "\n\n--------------------\n\n"
                        + visit_text
                    )
                else:
                    vals["x_wlw_visit_history"] = visit_text
    ...
    def _lead_name(self, v):
        company = v.get("company") or v.get("domain") or "WLW Besucher"
        interest = v.get("keywords") or v.get("products") or ""
        if interest:
            return "%s | %s" % (company, interest)
        return company
    ...
    def _history(self, v, signature):
        ...
        parts = []
        if v["date"]:
            parts.append(v["date"].strftime("%d.%m.%Y"))
        if v["keywords"]:
            parts.append("Suchbegriff: %s" % v["keywords"])
        if v["products"]:
            parts.append("Produkt: %s" % v["products"])
        if v["activity"]:
            parts.append("Aktivität: %s" % v["activity"])
